...
function getFilePath(folderId, nodeId, filename) {
    const folderPath = path_1.default.join(DOWNLOAD_DIR, folderId);
    return path_1.default.join(folderPath, `${nodeId}_${filename}`);
}
    ...
    return new Promise((resolve) => {
        try {
            ...
            stream.on('error', (err) => {
                ...
                const downloading = stmts.getFilesForFolder.all(folderId);
...
async function loadMegaFolder(url) {
    ...
    const folder = megajs_1.default.File.fromURL(url);
    ...
    const files = children.filter((child) => !child.directory);
...
async function retryFailedDownloads(folderId) {
    ...
    // Get failed files
    const failedFiles = stmts.getFailedFilesForFolder.all(folderId, 'failed');
    const pendingFiles = stmts.getFailedFilesForFolder.all(folderId, 'pending');
    ...
    if (!megaFolder) {
        ...
        megaFolder = megajs_1.default.File.fromURL(url);
...
addRoute('GET', '/folder', (_req, res) => {
    const folders = stmts.getAllFolders.all();
    const stats = stmts.getFileStats.all();
...
addRoute('GET', '/folder/:folderId', (_req, res, params) => {
    ...
    const files = stmts.getFilesForFolder.all(folderId);
    sendJson(res, 200, {
...
addRoute('GET', '/folder/:folderId/:nodeId', (_req, res, params) => {
    ...
    const file = stmts.getFile.get(folderId, nodeId);
    if (!file) {
...
async function handleRequest(req, res) {
    ...
    const parsedUrl = new url_1.URL(urlStr, `http://localhost:${PORT}`);
...
async function resumeDownloads() {
    ...
    const downloading = stmts.getFilesWithStatus.all('downloading');
    ...
    const pending = stmts.getFilesWithStatus.all('pending');
    ...
    for (const folderId of folderIds) {
        ...
        try {
            // Load folder from MEGA
            const url = `https://mega.nz/folder/${folderId}#${folder.folder_key}`;
            const megaFolder = megajs_1.default.File.fromURL(url);
            await megaFolder.loadAttributes();
            folderCache.set(folderId, megaFolder);
            const children = megaFolder.children || [];
            const filesMap = new Map();
...
function startRetryInterval() {
    setInterval(async () => {
        const rateLimited = stmts.getRateLimitedFolders.all();
